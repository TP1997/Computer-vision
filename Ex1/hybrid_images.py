...
wolft_highpass = wolft - wolft_lowpass #np.zeros(wolft.shape)  # replace me
##--your-code-ends-here--##
 
# Replace also the zero image below with the correct hybrid image using your filtered results
##--your-code-starts-here--##
hybrid_image = man_lowpass + wolft_highpass #np.zeros(man.shape)  # replace me
##--your-code-ends-here--##
...
##--your-code-starts-here--##
F_man = fftshift(fft2(man)) #np.ones(man.shape)  # replace me
F_man_lowpass = fftshift(fft2(man_lowpass)) #np.ones(man.shape)  # replace me
F_wolft = fftshift(fft2(wolft)) #np.ones(man.shape)  # replace me
F_wolft_highpass = fftshift(fft2(wolft_highpass)) #np.ones(man.shape)  # replace me
##--your-code-ends-here--##
