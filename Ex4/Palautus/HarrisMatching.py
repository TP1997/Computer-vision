...
import sys
sys.path.insert(0,'/home/tuomas/Python/DATA.ML.300/Ex4/Exercise 4/Python')
...
# Load images
os.chdir('/home/tuomas/Python/DATA.ML.300/Ex4/Exercise 4/Python')
...
distmat = np.zeros((npts1, npts2))
mean_p1 = np.mean(patches1, axis=2)
mean_p2 = np.mean(patches2, axis=2)
for i1 in range(npts1):
    for i2 in range(npts2):
        comp_p1 = patches1[:,:,i1] - mean_p1
        comp_p2 = patches2[:,:,i2] - mean_p2
        num = np.sum(comp_p1*comp_p2)
        denom = np.sqrt(np.sum(comp_p1**2) * np.sum(comp_p2**2))
        distmat[i1, i2] =  num / denom
...
sorted_ncc = np.sort(pairs[:,2], axis=0)[::-1] #-np.sort(-pairs[:,2], axis=0)
id_ncc = np.argsort(-pairs[:,2], axis=0)[::-1] #np.argsort(-pairs[:,2], axis=0)
