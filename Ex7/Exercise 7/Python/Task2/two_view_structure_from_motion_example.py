...
import os
import sys
sys.path.insert(0,'/home/tuomas/Python/DATA.ML.300/Ex7/Exercise 7/Python/Task2')
from utils import vgg_X_fromxP_lin, camcalibDLT
from mpl_toolkits.mplot3d import Axes3D

#%%
# Load the images (two views of the same scene)
os.chdir('/home/tuomas/Python/DATA.ML.300/Ex7/Exercise 7/Python/Task2')
...
# all of which can be found from the .mat-files in the working directory.
# Hint: loadmat
...
# Your task is to project the 3D corners in ABCDEFGH_w (currently non-homogeneous) 
# to images 1 and 2 using P1t and P2t, i.e. the calibrated camera matrices.
...
##-your-code-starts-here-##
proj1 = np.dot(P1t, np.hstack((ABCDEFGH_w, np.ones((8, 1)))).T)
cx1 = proj1[0] / proj1[-1] 
cy1 = proj1[1] / proj1[-1]

proj2 = np.dot(P2t, np.hstack((ABCDEFGH_w, np.ones((8, 1)))).T)
cx2 = proj2[0] / proj2[-1]
cy2 = proj2[1] / proj2[-1]
...
# Results are visualized using cyan lines. The cyan edges should be relatively close to the magenta lines from above.

# Convert the projected points back to non-homogeneous form and store into the given variables below.
##-your-code-starts-here-##
pproj1 = np.dot(P1, np.hstack((Xc.T, np.ones((8, 1)))).T)
pcx1 = pproj1[0] / pproj1[-1]
pcy1 = pproj1[1] / pproj1[-1]

pproj2 = np.dot(P2, np.hstack((Xc.T, np.ones((8, 1)))).T)
pcx2 = pproj2[0] / pproj2[-1]
pcy2 = pproj2[1] / pproj2[-1]
##-your-code-starts-here-##
